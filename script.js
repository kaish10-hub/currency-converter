...
async function convertCurrency() {
    ...
    try {
        ...
        const res = await fetch(
            `https://api.exchangerate.host/convert?from=${fromCurrency.value}&to=${toCurrency.value}&amount=${amountValue}`
        );
        const data = await res.json();
        const rate = data.rates[toCurrency.value];
        const result = amountValue * rate;
        convertedAmount.innerHTML = `${symbols[toCurrency.value] || ""}${result.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
        exchangeRate.innerHTML = `1 ${fromCurrency.value} = ${symbols[toCurrency.value] || ""}${rate.toFixed(4)} ${toCurrency.value}`;
...
function updateTime() {
    const now = new Date();
    lastUpdated.innerHTML = now.toLocaleString("en-IN", { datastyle: "medium", timestyle: "short" });
